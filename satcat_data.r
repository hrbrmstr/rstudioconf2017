#' This file ensures we have the data we need from the satellite data repository.
#' It caches the data, but this data will change as more satellites are launched
#' or crash/disintegrate. To refresh the data, just delete the cached data sets.
#' We store it in two files -- CSV & RDS -- to make it share-able outside of R
#' and preserve data types for use in R.

#' Defining this here for visibility. This is what you'll call to actually use the
#' resultant data structure. This really needs to be a package.

read_satellite_data <- function() readRDS(satcat_rds_path)
